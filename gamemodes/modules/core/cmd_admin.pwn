...
CMD:alogin(playerid)
{
	if(admin[playerid][admin_dostup])
	{
		...
		{
			format(format_string_admin, sizeof(format_string_admin), "%s %s вышел из административной панели.", admin_rank_name(playerid), users[playerid][u_name]);
...
{
	AdminCheck(1);
	if(sscanf(params, "s[96]", params[0])) return server_error(playerid, "Используйте: /(a)dmin [текст]");
...
{
	AdminCheck(1);
	show_dialog(playerid, d_admin_panel, DIALOG_STYLE_TABLIST_HEADERS, "Административная панель", "\
	Название раздела\tСтатус\n\
	{cccccc}- {ffffff}Список администрации\t{33AA33}ONLINE\n\
...
CMD:admins(playerid)
{
	...
	global_string[0] = EOS;
	strcat(global_string, "№\tНик\tСтатус\n");
	foreach(Player, i)
	{
		...
		number++;
		format(format_string, sizeof(format_string), "{cccccc}%i.\t{ffffff}%s\t%s\n", number, users[i][u_name], (GetPVarInt(i, "AFK") > 1)?("{B22222}AFK"):("{33AA33}В сети"));
		strcat(global_string, format_string);
	}
	if(!number) strcat(global_string, "Администрации нет в сети.");
	show_dialog(playerid, d_none, DIALOG_STYLE_TABLIST_HEADERS, !"Администрация онлайн", global_string, !"Закрыть", !"");
	// number = 0;
	return true;
}
CMD:recon(playerid, params[])
{
	...
	if(!admin[playerid][admin_hide])
	{
		format(format_string_admin, sizeof(format_string_admin), "[A] %s %s(%i) начал наблюдать за игроком %s(%i).", admin_rank_name(playerid), users[playerid][u_name], playerid, users[params[0]][u_name], params[0]);
		admin_chat(format_string_admin);
	}
	return true;
}
...
CMD:mute(playerid, params[]) 
{
	...
	if(admin[params[0]][u_a_level] >= 1) return server_error(playerid,"Игрок администратор");
	if(GetPlayerState(params[0]) == PLAYER_STATE_SPECTATING && observation[params[0]][observation_id] != INVALID_PLAYER_ID) return server_error(playerid, "Администратор в режиме наблюдения.");
	...
	format(format_string_logs, sizeof(format_string_logs), "Заблокирвоал чат игроку %s на %i минут. Причина: %s", users[params[0]][u_name], params[1], params[2]);
...
CMD:unmute(playerid, params[]) 
{
	...
	new format_string_logs[196];
	format(format_string_logs, sizeof(format_string_logs), "Разблокировал чат игроку %s. Причина: %s", users[params[0]][u_name], params[1]);
...
CMD:hp(playerid, params[]) 
{
	...
	server_message(playerid, "Вы восстановили состояние своего персонажа.");
	logs_admin(playerid, "Восстановил себя.", "/hp");
	return 1; 
}
...
CMD:goto(playerid, params[]) 
{
	...
	format(format_string_logs, sizeof(format_string_logs), "Телепортировался к игроку %s", users[params[0]][u_name]);
...
CMD:sethp(playerid, params[]) 
{
	...
	new format_string_logs[96];
	format(format_string_logs, sizeof(format_string_logs), "Изменил жизни игроку %s на %i", users[params[0]][u_name], params[1]);
...
CMD:kick(playerid, params[]) 
{
	...
	new format_string_logs[96];
	format(format_string_logs, sizeof(format_string_logs), "Кикнул игрока %s. Причина: %s", users[params[0]][u_name], params[1]);
...
{
	AdminCheck(2);
	if(sscanf(params, "s[128]", params[0])) return server_error(playerid, "Используйте: /ooc(/o) [Text]");
...
CMD:clearchat(playerid, params[]) 
{
	...
	SendClientMessageToAll(COLOR_BROWN, "Администрация "FULL_NAME" произвела очистку чата!");
	AdminChatF("[A] %s %s произвёл очистку чата.", AdminRankName(playerid), users[playerid][u_name]);
...
CMD:gethere(playerid, params[]) 
{
	...
	AdminChatF("[A] %s %s телепортировал к себе игрока %s[%i].", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0]);
...
CMD:ban(playerid, params[])
{
	...
	if(GetPlayerState(params[0]) == PLAYER_STATE_SPECTATING && observation[params[0]][observation_id] != INVALID_PLAYER_ID) return server_error(playerid, "Администратор в режиме наблюдения.");
	if(!admin[playerid][admin_fulldostup] && admin[params[0]][admin_protection]) return server_error(playerid, "Администратор защищен.");
	if(params[1] < 7 || params[1] > 365) return server_error(playerid, "Кол-во дней должно быть от 7-и до 365-ти");
...
CMD:skin(playerid, params[])
{
	...
	//if( params[0] == 74) return server_error(playerid, "Неверный номер скина или скин запрещен.");
...
CMD:veh(playerid, params[])
{
	...
	AdminChatF("[A] %s %s создал транспорт. (ID: %i | Model: %i)", AdminRankName(playerid), users[playerid][u_name], adm_veh, params[0]);
	return true;
}
CMD:delveh( playerid, params [ ])
{
	...
	AdminChatF("[A] %s %s удалил транспорт. (ID: %i)", AdminRankName(playerid), users[playerid][u_name], veh_id);
...
CMD:offban(playerid, params[])
{
    ...
    if(!strcmp(name, "Noir", true) || !strcmp(name, "Felix", true)) return server_error(playerid, "Главную администрацию нельзя заблокировать оффлайн");
...
CMD:weather(playerid, params[]) 
{
	...
	AdminChatF("[A] %s %s сменил погоду.", AdminRankName(playerid), users[playerid][u_name]);
...
CMD:reprimand(playerid, params[])
{
	...
	if(r)
	{
		...
		{
			SCMG(playerid, "Администратор %s(%i) заморожен, свяжитесь со старшей администрацией.", users[params[0]][u_name], params[0]);
			cache_delete(temp_sql);
			return true;
		}
		if(!strcmp(str, "+", false))
			...
			case 0, 1:
				{
					...
					m_query(str_sql);
					AdminChatF("[A] %s %s выдал администратору %s[%i] выговор. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0], params[2]);
			...
			case 2:
				{
					...
					m_query(str_sql);
					AdminChatF("[A] %s %s выдал администратору %s[%i] выговор. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0], params[2]);
					AdminChatF("[A] %s %s временно заморожен в административной панели. Причина: Выговор 3 из 3", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0], params[2]);
			...
			case 1, 2:
				{
					m_format(str_sql, sizeof(str_sql), "UPDATE "TABLE_ADMIN" SET `admin_vig` = `admin_vig` - '1' WHERE `admin_name` = '%s'", users[params[0]][u_name]);
					m_query(str_sql);
					AdminChatF("[A] %s %s снял администратору %s[%i] выговор. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0], params[2]);
			...
			case 0..2:
				{
					...
					m_query(str_sql);
					AdminChatF("[A] %s %s выдал клану %s(%i) выговор. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], clan[params[0]][c_name], params[0], params[2]);
			...
			case 1..3:
				{
					m_format(str_sql, sizeof(str_sql), "UPDATE "TABLE_CLAN" SET `c_reprimand` = `c_reprimand` - '1' WHERE `c_id` = '%i'", params[0]);
					m_query(str_sql);
					AdminChatF("[A] %s %s снял клану %s(%i) выговор. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], clan[params[0]][c_name], params[0], params[2]);
...
CMD:asetcolor(playerid, params[])
{
	...
	if(admin[params[0]][admin_dostup] != 1) return SCMASS(playerid, "Игрок %s не залогинен в админ панели.", users[params[0]][u_name]);
	...
	m_query(str_sql);
	AdminChatF("[A] %s %s изменил цвет администратору %s на {%s}%s", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], prefix_colors[params[1]-1][1], prefix_colors[params[1]-1][0]);
...
CMD:reloot(playerid, params[])
{
	...
	AdminChatF("[A] %s %s изменил время релута, релут через %i минут(у)", AdminRankName(playerid), users[playerid][u_name], params[0]);
	return true;
}
CMD:asetrank(playerid, params[])
{
	...
	m_query(str_sql);
	AdminChatF("[A] %s %s изменил название ранга администратору %s на %s", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], admin[playerid][admin_rank]);
...
CMD:afreezeoff(playerid, params[])
{
	...
	if(r)
	{
		...
		if(!strcmp(str, "+", false))
		{
			...
			m_query(str_sql);
			AdminChatF("[A] %s %s заморозил администратора %s[%i]. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0], params[2]);
		...
		if(!strcmp(str, "-", false))
		{
			...
			m_query(str_sql);
			AdminChatF("[A] %s %s разморозил администратора %s[%i]. Причина: %s.", AdminRankName(playerid), users[playerid][u_name], users[params[0]][u_name], params[0], params[2]);
...
{
	if(!admin[playerid][admin_fulldostup]) return server_error(playerid, "Доступ ограничен.");
...
CMD:givegun(playerid, params[]) 
{
	...
	AdminChatF("[A] %s %s выдал оружие %i и %i патрон игроку %s[%i].", AdminRankName(playerid), users[playerid][u_name], params[1], params[2], users[params[0]][u_name], params[0]);
...
CMD:loot(playerid, params[]) 
{
	...
	if(!admin[playerid][admin_hide]) AdminChatF("[A] %s %s создал предмет %s в колличестве %i шт (%f, %f, %f).", AdminRankName(playerid), users[playerid][u_name], loots[params[0]][loot_name], params[1], l_c[0], l_c[1], l_c[2]);
	static str_logs[96];
